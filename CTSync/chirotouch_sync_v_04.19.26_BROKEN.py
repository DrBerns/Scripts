...
# v_04.19.26: JOIN chain extended through PaymentClaims → Claims → ClaimLines
# to pull Modifier1..4 and Units for Automation A (design v_04.19.26.2 §4.2, §4.3).
# BeginDate tie-breaker handles multi-visit (ClaimID, ProcedureCode) duplicates.
# Upsert key is ct_transaction_id (Transactions.ID) so re-runs are safe and will
# backfill existing rows with new fields on the first v_04.19.26 run.
CHARGES_QUERY = """
SELECT
    ...
    p.FirstName + ' ' + p.LastName  AS patient_name,
    p.ChartNo                       AS chart_no,
    -- v_04.19.26: modifier + units from ClaimLines
...
LEFT JOIN PaymentClaims pc ON pc.ID               = t.PaymentClaimID
LEFT JOIN Claims        c  ON c.ID                = pc.ClaimID
LEFT JOIN ClaimLines    cl ON cl.ClaimID          = c.ID
                          AND cl.ProcedureCode    = t.Code
                          AND cl.BeginDate        = t.TranDate
WHERE t.TranType = 'C'
...
def build_patient_lookup():
    ...
    lookup = {}
    for r in pat_tbl.all(fields=["fld4ogKYb3HOjcm4w"]):
...
def charge_fields(row, patient_lookup):
    ...
    NEW in v_04.19.26 (aligned with Automation A design v_04.19.26.2):
      ...
      - Linked Patient (fldbwgMmbakZPWCBv): Airtable record ID from patient_lookup
    """
    ...
    f = {
        ...
        # Units (number, integer) — from ClaimLines.Units; defaults to 1 if NULL or 0
        "fldwILSiViRe6PdjE":  int(row.units) if row.units and row.units > 0 else 1,

        # Linked Patient (multipleRecordLinks → Patients) — resolved via lookup; None if unresolved
        "fldbwgMmbakZPWCBv":  linked_patient,
